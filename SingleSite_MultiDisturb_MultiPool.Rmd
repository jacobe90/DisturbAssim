...
for(t in 1:NT){
  ...
  #plot(jdat)
  jdat <- rjags::coda.samples(mod,variable.names=c("X","rho","x"),n.iter=20000) 
  #print(jdat[0,])
  ...
  if (t == 15) {
    ...
    for (d in 1:3) {
      ...
      mu.F[d,] <- muf[,d]
      cov.F[d,,] <- solve(pf[,,d])
      Na.d <- Xa[d.class.post == d,]
      Na.d.mu <- colMeans(Na.d)
      Na.d.Sig <- cov(Na.d)
      mu.A[d,] <- Na.d.mu
      cov.A[d,,] <- Na.d.Sig
    }
    save(distprobs, cw.obs, R_H, mu.F, cov.F, mu.A, cov.A, file="multidisturb-ex-values.RData")
  }
