...
nt = 365*NT # number of total timesteps
time = 1:nt # array of all timestep indices

## drivers
nd   = 31 ## number of driver ensembles
PAR <- matrix(NA,nd,nt) # (nd (num driver ensembles) x nt (total number of timesteps))
for(i in 1:nd){
  PAR[i,] <- VSEMcreatePAR(time)  # create the PAR driver ensemble  
}
...
## spin up
ne = 1000
met = sample.int(nd,ne,replace=TRUE) # array of met indices: 1 x ne (num ensemble members)
par = sample.int(np,ne,replace=TRUE) # array of param indices: 1 x ne (num ensemble members)
Na <- matrix(NA,ne,3) # variable which stores the ensemble of state data after the spin-up
for(i in 1:ne){
  Na[i,] = VSEM(as.numeric(Theta[par[i],]),PAR = PAR[met[i],])[nt,2:4] # VSEM returns a len(PAR) x 4 (state vars) = nt x 4 array
  # grab data from the last element (final timestep of spin-up), and state vars 2-4 (Cv, Cs, Cr)
}
Na.orig = Na # ensemble of state data after the spin-up
par(mfrow=c(1,3))
...
```{r}
## hard coded 
Q = diag(c(0.1,0.1,0.1)) ## process error (num state vars (nS) x num state vars (nS))
R = diag(c(.5,10,1))      ## observation error
R = as.matrix(Matrix::bdiag(R,R))  ## nsite = 2 (block diagonal R matrix)
THETA <- as.matrix(Theta[par,]) ## ensemble parameters (ne x nP (parameter dimension))
THETA = abind::abind(THETA,THETA,along = 3)  ## n=2 sites (ne x nP (parameter dimension) x 2)
...
## pseudodata
nsite = 2
No = matrix(NA,NT,3*nsite)                   ## storage for observed state (at each site, for each year)
Ntrue = matrix(NA,NT,3*nsite)                ## storage for latent "true" state (at each site, for each year)
true = 42                                    ## reference ensemble member
D.true = matrix(0,nsite,NT)                  ## true disturbance timeseries (for each site, for each year)
D.true[1,10] = 1                             ## prescribe a disturbance (only at tstep 10 for site 1)
refpar = matrix(as.numeric(Theta[par[true],]),nsite,ncol(Theta),byrow = TRUE) ## grab the "true" parameters (use for both sites)
refpar[,9:11] = Na[true+(1:nsite)-1,]        ## set initial biomass pools (biomass for first site = refpar[1,9:11] = Na[true,]) (biomass for second site = refpar[2,9:11] = Na[true+1,]). Different for each site.
print(refpar)
ref.f = list()
for(t in 1:NT){
  ...
  Ntrue[t,] = unlist(ref.f) # expand all sites into one vector
  No[t,] = tobit(mvtnorm::rmvnorm(1,Ntrue[t,],R)) ## observation error (add to all sites at once)
}
par(mfrow=c(3,1))
plot(No[,1])
lines(No[,4],col=2)
...
Na <- NaK <- abind::abind(Na.orig,
                          Na.orig[sample.int(ne,ne),],  ## resample IC for site 2 (without replacement)
                          along = 3)
NfK = Nf = Na*NA # ne (num ensemble members) x 3
THETAK <- THETA # ne x nP (parameter dimension) x 2 (num sites)
d.prior = matrix(nrow=ne,ncol=2) # will store whether each site of each ensemble member is disturbed
muf <- matrix(NA,3*nsite,4) # (nS*nsite x nD^nsite) forecast mean for each scenario, and there are 2^nsite=4 disturbance scenarios
pf  <- array(0,c(3*nsite,3*nsite,4)) ## 3rd variable is class (forecast precision for each scenario)
for(t in 1:NT){
  
  tsel <- (t-1)*365 + 1:365               ## Select days from driver files
  
  ## update IC
  THETA[,9:11,]  <- Na    ## Multinomial (note we are updating with state data for two sites)
  THETAK[,9:11,] <- NaK  ## EnKF w/ disturbance
  
  ## Forecast 
  for(i in 1:ne){
    for(s in 1:nsite){
      Nmu  =  VSEM(THETA[i,,s], PAR = PAR[met[i],tsel])[365,2:4] # use same met data across sites 
    ...
    if(length(sel.dist)> (3*nsite)){
      for(j in sel.dist){                      ## run disturbance model
        switch (as.character(d),
                "2" = {Nf[j,,1] = disturbance(Nf[j,,1]) # disturb at only site 1
                NfK[j,,1]= disturbance(NfK[j,,1])
                },
                "3" = {Nf[j,,2] = disturbance(Nf[j,,2]) # disturb at only site 2
                NfK[j,,2]= disturbance(NfK[j,,2])
                },
                "4" = {Nf[j,,1] = disturbance(Nf[j,,1]) # disturb at both sites 1 and 2
      ...
      muf[,d]  = colMeans(cbind(Nf[sel.dist,,1], Nf[sel.dist,,2])) # compute mean state vector of disturbed ensemble members by site, and concatenate them into a vector to store as the forecast mean
      pfTMP    = try(solve(var(cbind(Nf[sel.dist,,1],Nf[sel.dist,,2])))) # concatenate state variables for each site for each ensemble member, compute sample precision of these vectors
    } 
    if(sum(sel.dist) <= (3*nsite) || "try-error" %in% class(pfTMP)) { 
  ...
  priors <- list(N = 4,  ## 2 sites x 2 disturbance classes
                 p   = table(d.class)/ne,         ## disturbance rate (compute counts of different types of disturbance and then normalize)
                 muf = muf,    ## forecast means
                 pf  = pf      ## forecast precisions
  )
  print(priors$p)
  ...
  Lp = NA
  for(i in 1:4){ Lp[i] = mvtnorm::dmvnorm(No[t,],muf[,i],solve(pf[,,i]))} # compute likelihood of observation coming from each disturbance scenario
  rho.IC = rep(0,4); rho.IC[which.max(Lp)] = 1; rho.IC[1] = NA # initial rho for MCMC: a one-hot for max likelihood scenario (set rho.IC[1] = NA since this is deterministic in the JAGS code)
