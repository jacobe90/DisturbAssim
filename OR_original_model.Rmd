...
SSSD.model <- function(Y,R,Q,PAR,Xic,THETA,dist){
  ...
  for(t in 1:NT){
    ...
    # d.forecast <- rbinom(ne, 1, dist$p.dist) ## prior on disturbance status (set ~10% of ensemble members to be disturbed) Question: Why not use dist$p.dist? Why not disturb random ensemble members
    ...
    if(length(update$H) > 0){ ## data observed, run filter
      ...
      ca = dat[,"D"] # disturbance classes of analysis samples
...
```
```{r}
# function to run data assimilation given R / Q / P multipliers
# Rmult -- multiplier for observation error, R
# Qmult -- multiplier for process error, Q
# Pmult -- multiplier for disturbance probability
runDA <- function(Rmult, Qmult, Pmult, siteIndices) {
...
for (i in seq_len(nrow(expGrid))) {
  ...
  runDA(row$Rmult, row$Qmult, row$Pmult, testRun) # run data assimilation experiment
}
...
print(Rs)
print(Qs)
plot(rev(expGrid$Rmult),div.cov,col=rev(expGrid$Qmult),log="x",xlab="Variance Multiplier",ylab="Filter Coverage")
#legend("topright",legend=paste0(Qmult[1:2],"x"))

div.median = sapply(stats,"[[","div.median")
plot(rev(expGrid$Rmult),div.median,col=rev(expGrid$Qmult),log="x",xlab="Variance Multiplier",ylab="Median Divergance")
legend("topleft",legend=paste0(Qmult[1:2],"x"))

div.slope = sapply(stats,function(x){coef(x$div.reg)[2]})
plot(rev(expGrid$Rmult),div.slope,col=rev(expGrid$Qmult),log="x",xlab="Variance Multiplier",ylab="Divergance Slope")
## Disturbance @ default
...
## settings
site = 197 # 188
exp.name = "P1Q1R1" #"P1Q1R4"  ## manually run for each experiment **
