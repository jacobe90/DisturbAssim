---
title: "SingleSite_MultiDisturb_SinglePool"
...
#  X <- x%*%rho
  Y[3] ~ dmnorm(X[3],R[3,3])                       ## Observation error
}"
...
for(t in 1:NT){
  ...
  for(d in 2:3){  ## loop over disturbance classes
    ...
    if(sum(sel.dist) <= 3 || "try-error" %in% class(pf[,,d])) {   ## backup values if the ensemble stats fail
  ...
  Lp = NA
  for(i in 1:3){ Lp[i] = mvtnorm::dmvnorm(No[t,],muf[,i],solve(pf[,,i]))} # likelihoods of current observation under each type of disturbance (and no disturbance)
  rho.IC = rep(0,3); rho.IC[which.max(Lp)] = 1; rho.IC[1] = NA # pick rho to be type of disturbance (or no disturbance) with max likelihood of occuring. (set rho.IC[1] = NA?)
  print(rho.IC)
  ...
  #plot(jdat)
  jdat <- rjags::coda.samples(mod,variable.names=c("X","rho","x"),n.iter=40000) 
  #print(jdat[0,])
  ...
  if (t == 15) {
    ...
    mu.F <- array(NA, dim = c(3, 3))
    cov.F <- array(NA, dim = c(3, 3, 3))
    mu.A <- array(NA, dim = c(3, 3))
    cov.A <- array(NA, dim = c(3, 3, 3))
    w.A <- array(NA, dim=c(3))
    
    for (d in 1:3) {
      w.A[d] <- length(which(d.class.post == d)) / length(d.class.post)
      Xa <- dat[,Xsel]
      mu.F[d,] <- muf[,d]
      cov.F[d,,] <- solve(pf[,,d])
      Xa.d <- Xa[d.class.post == d,]
      Xa.d.mu <- colMeans(Xa.d)
      Xa.d.Sig <- cov(Xa.d)
      mu.A[d,] <- Xa.d.mu
      cov.A[d,,] <- Xa.d.Sig
    }
    save(distprobs, cw.obs, R_H, mu.F, cov.F, mu.A, cov.A, w.A, file="multidisturb-ex-values.RData")
  }
...
```
```{r, fig.asp=1,fig.width=5}
# plot ensemble members after adjustment and color by forecast disturbance class
d.class <- apply(Forecast[[10]]$d.prior, 2, which.max)
cols <- c("1" = "red", "2" = "blue", "3" = "green")

plot(Analysis[[10]]$Na[,1], Analysis[[10]]$Na[,3],
     pch = 16,
     col = cols[d.class],
     ylim = range(Analysis[[10]]$Na[,3]),
     xlim = range(Analysis[[10]]$Na[,1]),
     xlab = pool.names[1],
     ylab = pool.names[3],
     cex.lab = 1.25,
     asp = 1)
...
legend("topright",               # position of legend
       legend = names(cols),    # class indices
       col = cols,              # colors
       pch = 16,                # same point shape as plot
       title = "Forecasted Disturbance")
```
