...
SSSD.model <- function(Y,R,Q,PAR,Xic,THETA,dist,updateQ=TRUE){
  ...
  for(t in 1:NT){
    ...
    if(sum(!sel)>2){ # if there are more than 2 disturbed ensemble members
      ...
      CfD = var(Nf[!sel,])
      pfD = try(solve(CfD)) # compute sample covariance of disturbed ensemble members
    } 
    ...
    if(length(H) == 1){ ## data observed, run filter
      ...
      pD_star = pD * dnorm(Y[t,H], mat_H %*% mufD, mat_H %*% solve(pfD) %*% t(mat_H) + R_H) # posterior probability of disturbance
      pN_star = (1-pD) * dnorm(Y[t,H], mat_H %*% mufN, mat_H %*% solve(pfN) %*% t(mat_H) + R_H) # posterior probability of not being disturbed
...
for(i in seq_along(exps)){
  if(is.null(elist[[exps[i]]])){
    ...
    for(j in seq_along(esel)){
      ...
      distrate[k]=test$Analysis[[t04]]$mu["D"] # posterior probability of disturbance
